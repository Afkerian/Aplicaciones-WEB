...
var controller={
    ...
    getLibroNombre:function(req,res){
        var nombre=req.params.nombre;
        console.log("nombre: "+nombre);
        if(nombre==null) return res.status(404).send({message:'El nombre no existe'});
        
        Libro.find({nombre},(err,libro)=>{
            if (err) return res.status(500).send({message:'Error al recuperar los datos'});
            if(!libro) return res.status(404).send({message:'El nombre no existe'});
            return res.status(200).send({libro});
        })
    }
